...
let rec translate_term (e : ty expr) : P.term =
  let open P in
  let open PH in
  ...
  | Var (s, (cat_t,_)) ->
        begin
          match cat_t with
          | Local -> tvar (qualid [ s ])
          | State | Input | Output ->  tapp ~loc (qualid [ s ])
...
let [@warning "-4"] pterm_of_pltl (f :  (base_spec_t,_) labeled Pltl.pltl_) : P.term = 
  match f with 
  | PLTL_Atom x -> pterm_of_fol x.value 
  | Pltl.PLTL_True -> PH.term Ttrue
  | Pltl.PLTL_False -> PH.term Tfalse
  | Pltl.PLTL_Unary (_, _) ->  failwith "pltl not implemented"
  | Pltl.PLTL_Binary (_, _, _) -> failwith "pltl not implemented"
...
module
  ...
  type temp_spec = 
    ((FrontSig.temp_f_prop, ty, FrontParser.SharedSyntax.base_ty)  
      FrontParser.Pltl_spec.temp_spec_t, FrontSig.temp_f_prop)  labeled  and
  type in_spec = (((Shared.ty, Shared.base_ty) fol_t, fol_data) U.labeled, formula_data) labeled cnf and
  type out_pgrm = P.mlw_file 
= struct

  type local_spec = base_spec_t
  type temp_spec = ((FrontSig.temp_f_prop, ty,
             FrontParser.SharedSyntax.base_ty)
            FrontParser.Pltl_spec.temp_spec_t,
            FrontSig.temp_f_prop)
           labeled
