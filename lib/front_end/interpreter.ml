...
let eval_bop op (x: value) (y: value) : value = 
  ...
  let do_if_var_exists p f id = 
    if Bindings.mem id p.prog_decls.env_variables then
      f ()
...
let rec eval_rexpr p (e: _ expr) : value = 
  ...
  | UnOp (ENot, e) -> LBool (get_bool (aux e) |> not)
  | BinOp bop -> 
    let e1 = aux bop.left in
    let e2 = aux bop.right in
    eval_bop bop.op e1 e2
in aux e
...
| Var s ->  
  if Bindings.exists (fun id (c,_) -> id = s && c = State) pgrm.prog_decls.env_variables then 
...
let eval_stmts p (s: _ stmt list) : unit =  
  ...
  | Emit (e, id) -> 
    ...
    if Bindings.exists (fun o (c,_) -> id = o && c = Output) p.prog_decls.env_variables then 
...
module ConsoleBridge : IOBridgeSig = struct
  ...
  let send_outputs state o = 
    ...
    List.iter (fun (o, _) -> Format.printf "%s: %a@." o (fun fmt -> pp_value fmt) (Hashtbl.find env o)) o ;

end


let eval_pgrm (p: base_program) (module B : IOBridgeSig) = 
  Bindings.iter (fun v (cat,_) -> match cat with Input | State | Output -> Hashtbl.add env v LEmpty | _ -> ()) p.prog_decls.env_variables;
  ...
  let rec run (n: _ node) : unit =  
    ...
    let env_variables = Bindings.(merge (fun _ _ v2 -> v2
    ) (of_list n.node_variables) p.prog_decls.env_variables) in
    let local_p = {p with prog_decls = {env_variables}} in
