...
and path = Var of string | ArrayCell of path * int
    ...
    | Var (id, _) ->
        do_if_var_exists p
          (fun () ->
            ...
            with _ -> failwith @@ "unknown input/variable " ^ id)
          id
    | ArrayCell (id, n) -> (
...
and eval_lexpr p (e : _ expr) : path =
  let aux (e : _ expr) =
    ...
    | Var (id, _) -> Var id
    | ArrayCell (id, idx) ->
        let id = eval_lexpr p id in
        ArrayCell (id, get_int (eval_rexpr p idx))
  ...
  let rec run (n : _ node) : unit =
    let rec do_transition p cleanup t =
      let cleanup_and_find_node = function
        | Some n -> (
            cleanup ();
            try run (find_node n p.prog_nodes)
            with Not_found -> failwith @@ "unknown node " ^ n)
        | None ->
            cleanup ();
            run n
      in
      ...
      | (Some guard, new_node) :: t ->
          ...
          else do_transition p cleanup t
    in
    ...
    let local_p =
      {
        ...
        prog_decls =
          {
            p.prog_decls with
            env_variables = n.node_variables @ p.prog_decls.env_variables;
          };
      }
    in
