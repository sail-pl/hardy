...
module Functional : S = struct
  ...
  let subst (f : string t) : string t =
   fun m ->
    ...
    ( sub_atom_in_str
        (fun s ->
          let a = get s in
          Format.asprintf "%a" (pp_fol pp_ty) a)
        f,
      (cnt, m) )
  ...
  let add_and_get (atom : ty fol_t) : (string * string) t =
    let key = Hashtbl.hash (Format.asprintf "%a" (pp_fol pp_ty) atom) in
...
module Imperative () : S with type 'a t = 'a = struct
  ...
  let subst =
    sub_atom_in_str (fun s ->
        let _, inv = get s in
        Format.asprintf "%a" (pp_fol pp_ty) inv)

  let add_and_get (atom : ty fol_t) =
    let key = Format.(asprintf "%a" (pp_fol pp_ty) atom) |> String.hash in
    let label = Format.sprintf "f_%i" key in
