...
let translate_program (i:info) (p : program) : P.mlw_file = 
  ...
  let funs = PG.fold_vertex (fun v l -> (
      let in_e = PG.pred_e a v in
      let out_e =  PG.succ_e a v in
  
      (* provide init post-condition for first node *)
      let extra_req = Option.(bind p.prog_setup @@ fun x -> if PG.is_start_node v then x.setup_ensures else None) in
    
      let specs = make_prod_spec p.prog_env.env_input in_e out_e extra_req |> to_spec in
