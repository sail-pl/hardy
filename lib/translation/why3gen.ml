...
let rec translate_expression ({value=e;loc}:expr) : Ptree.expr = 
  ...
  | Var s -> 
    ...
    | Var,_ | Output,_ -> let deref = Ptree_helpers.qualid [Ident.op_prefix "!"] in 
      Ptree_helpers.eapp ~loc deref Ptree_helpers.[[s] |> qualid |> evar]
    end
    
  | Read s -> Ptree.Easref (Ptree_helpers.qualid [s]) |> Ptree_helpers.expr ~loc

  | BinOp (e1,binop,e2) -> Ptree_helpers.eapp ~loc (translate_binop binop) [translate_expression e1; translate_expression e2]
  ...
  let rec translate_term (e:expr) : Ptree.term =
    ...
    | Var s ->     
      ...
      | Var,_ | Output,_ -> let deref = Ptree_helpers.qualid [Ident.op_prefix "!"] in 
      Ptree_helpers.tapp ~loc deref Ptree_helpers.[[s] |> qualid |> tvar]
      end
  
    | Read s ->       
      Ptree.Tasref (Ptree_helpers.qualid [s]) |> Ptree_helpers.term ~loc
  
    | BinOp (e1,binop,e2) -> Ptree_helpers.tapp ~loc (translate_binop binop) [translate_term e1; translate_term e2]
  
    
  let rec translate_statements (tr_form : invariant -> Ptree.term) (s: stmt list)  : Ptree.expr = 
    let open Ptree in 
    ...
    let get_pty ty = 
      let ty = string_of_ty ty in Ptree.(PTtyapp ((Ptree_helpers.qualid [ty]),[]))
    ...
    let rec pterm_of_fol ({value=f;loc}:fol) : Ptree.term = 
      ...
      | Exists (v,f) ->  Tquant (
        ...
        let mk_fun name spec body = 
          ...
          |> expr 
          |> fun m -> Dlet (ident name, false, Expr.RKnone, m)
        ...
        let make_setup (setup : setup option) = 
          ...
          | Some s ->
            begin
              ...
              let spec = match Option.(map pterm_of_fol s.setup_ensures)  with
                | None -> empty_spec
                | Some f -> 
...
let to_spec (l : fol list hoare_pair list) : Ptree.spec list = 
  List.map 
        (fun h -> 
          ...
          let post = List.map (fun e -> Ptree_helpers.(pat) Pwild,pterm_of_fol e) h.ensures  in
