...
let make_prod_spec (input : (string*ty) list) (in_e : PG.E.t list) 
  (out_e : PG.E.t list) (init_post: fol option): fol list hoare_pair list =
    ...
    let m = 
      ...
      List.fold_left 
        (fun m e -> let l = PG.E.label e in M.add_to_list l.requires l.ensures m) 
        M.empty out_e 
    in
        ...
        (fun (k : bform) (d: bform list) s -> 
          let requires = 
            ...
            in let with_init = 
                ...
                Option.fold init_post ~none:exists_dij ~some:(fun i -> or_fol i exists_dij)
            in [and_fol with_init (bform_to_fol k)]
          and ensures = 
