...
let make_loop (body:Ptree.expr) inv = 
  ...
  let translate_program _ (p : program) : P.mlw_file = 
    ...
    let sp_pre = Option.(map translate_formula p.prog_spec.requires |> to_list) in
    let inv = Option.map translate_formula p.prog_spec.ensures in
    let sp_post = Option.(map(fun i -> Loc.dummy_position, [pat Pwild, i]) inv |> to_list) in
    ...
    let loop = make_loop body (Option.to_list inv) in
