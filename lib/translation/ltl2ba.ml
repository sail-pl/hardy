(* https://github.com/EmileRolley/ltl2ba/blob/main/src/ltl2ba.ml *)
...
let add_acceptant_trans ((s0,get_vertex) : state * (state -> Ba.vertex))
    ...
    let acceptant_formulas, sigma =
      ...
      |> List.fold_left (fun (fs, sigs) e -> match Ba.E.label e with
          | `Acceptant (f, s) when f = fs -> 
              Ba.remove_edge_e ctx.ba e;
              ( fs, if FormulaSet.is_empty s then s else FormulaSet.union s sigs )
...
let add_normal_trans ((s0,get_vertex) : state * (state -> Ba.vertex)) (ctx : translating_context) (s : state) =
  let v0 = get_vertex s0
  and sigma = ctx.get_sigma s
  and v1 = get_vertex (Al.next s) in
  if not @@ (
    Ba.find_all_edges ctx.ba v0 v1 |> List.exists (fun e ->
    match Ba.E.label e with
    | `Acceptant (_, s) -> s = sigma
    | _ -> false)
  )
  then 
...
let add_states_from (s0 : state) ((ctx, red_states) : translating_context * Al.red_states) 
  : translating_context * Al.red_states = let open Al in
  ...
  then (
    ...
    |> add_acceptant_trans (s0,get_vertex) ctx s0;
    ...
    then
      ...
      |> StateSet.iter (fun s ->
        let edge =
          ...
          then 
            let sigma = ctx.get_sigma s in `Normal sigma
          else 
      ...
      FormulaMap.iter (fun phi states ->
        StateSet.diff (red_states_union red_states_from_s0) states
          |> StateSet.iter (fun s -> add_acceptant_trans (s0,get_vertex) ctx s [ phi ]);
          StateSet.iter (add_normal_trans (s0,get_vertex) ctx) states
        ) red_states_from_s0.marked_by;
        ( ctx, 
        { 
          all = 
            StateSet.empty
            |> StateSet.fold (fun s next_red_state -> StateSet.add (next s) next_red_state)
                red_states_from_s0.all
            |> StateSet.union red_states.all; 
          marked_by = 
            FormulaMap.map (fun states ->
              StateSet.fold (fun s next_red_state -> StateSet.add (next s) next_red_state)
...
let print_automata (file:string) (a:Ba.t)=
  Out_channel.with_open_bin file (fun f ->
    DotPrinter.output_graph  f a
  )