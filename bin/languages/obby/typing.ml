...
open Syntax.Fol
open FrontSig
open FrontParser.Obby.Ltl
open FrontParser.Obby.Syntax
open FrontParser.Specification
...
: Typing with type in_t = (Spec.parsed_temp_spec_t, Spec.parsed_spec_t) program
    and type out_t = ((((InstantSyntax.instant option * Syntax.Shared.ty), Syntax.Shared.base_ty, temp_f_prop) Spec.temp_spec_t, temp_f_prop) labeled,
        Spec.base_spec_t)  program
    and type out_local_spec = Spec.base_spec_t

= struct
    type in_temp_spec = Spec.parsed_temp_spec_t
    type out_temp_spec = (((InstantSyntax.instant option * Syntax.Shared.ty), Syntax.Shared.base_ty, temp_f_prop) Spec.temp_spec_t, temp_f_prop) labeled
    (* Instant.instant should always be None, this just allows for uniform processing  *)

    type in_local_spec = Spec.parsed_spec_t
    type out_local_spec = Spec.base_spec_t

    type in_t = (Spec.parsed_temp_spec_t, Spec.parsed_spec_t) program
    type out_t = (out_temp_spec, out_local_spec) program
    

    let type_pgrm (p: in_t) : out_t = failwith "todo"
    ...
    let set_expr_type : 'a 'b. ('a -> 'b) -> ty Bindings.t -> (unit,'a) expr -> (unit,'b * ty) expr = fun f b x ->
        map_expr Fun.id Fun.id (fun (id,t) -> id,(f t,fail_if_no_bindings id b)) x in
    ...
    let prog_spec : (out_temp_spec list, unit) hoare_triple = 
        let type_spec checks = 
            List.map (fun (f_ltl:parsed_temp_spec_t) : out_temp_spec ->
                let f_ltl : ((InstantSyntax.instant option * ty), base_ty, temp_f_prop) temp_spec_t = 
                    map_ltl_pred (fun f_fol : ((InstantSyntax.instant option * ty, base_ty) fol_t, temp_f_prop) labeled -> 
                        ...
                        if is_static_prop prop then 
                            Format.asprintf "temporal formula '%a' does not contain any program variables" 
                                Printer.(pp_fol (pp_pred (pp_exp Format.pp_print_nothing (fun fmt (s,(t,_)) -> pp_hist fmt (s,t)))) (Format.pp_print_option pp_base_ty)) fol |> failwith
                        ;
    ...
    and prog_main = 
        let main_body = type_fun (fun () -> None) bindings p.prog.prog_main.main_body
        and main_loop_inv = List.map (type_prog_spec (fun () -> None) bindings) p.prog.prog_main.main_loop_inv in
