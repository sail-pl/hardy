...
module Cli : CliSig = functor () -> struct
  open Arg
  let usage_msg = Printf.sprintf "Usage : %s --ltl2ba <ltl2ba exe> <file> [-v] [-pltl]" (Sys.argv.(0) |> Filename.basename)
  let input_file = ref "" 
  ...
  let parseLtl2baPath p = 
    if not @@ Sys.file_exists p then 
      raise @@ Bad "Can't stat ltl2ba program" 
    else ltl2baPath := p
  ...
  let get_input_file f = 
    if not @@ Sys.file_exists f then 
      ...
      if !input_file = "" then input_file := f 
